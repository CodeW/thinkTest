...
{
    $e = array();
    if (APP_DEBUG)		// link: ThinkPHP/ThinkPHP.php
	{
        //调试模式下输出错误信息
        if (!is_array($error))
		{
	...
	{
        $error_page = C('ERROR_PAGE');		// link: C()
        if (!empty($error_page))
		{
            redirect($error_page);			// link: self/redirect
        }
		else
		{
            if (C('SHOW_ERROR_MSG'))		// link: C()
...
function redirect($url, $time = 0, $msg = '')
{
    ...
    if (!headers_sent())	// 检查是否已经发送有头信息要发送
	{
        // redirect
        if (0 === $time)
		{
...
/**
 * 获取客户端IP地址
 * @param integer $type 返回类型 0 返回IP地址 1 返回IPV4地址数字（十进制表示）
 * @return mixed
 */
