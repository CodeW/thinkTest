...
class Think
{
      ...
      switch ($errno)
	  {
          ...
          case E_USER_ERROR:
            ...
            if (C('OUTPUT_ENCODE'))
			{
                ...
                if (empty($zlib))
					ob_start('ob_gzhandler');	// 对缓存的数据进行gzip压缩后再发送给客户端
            }
            $errorStr = "$errstr ".$errfile." 第 $errline 行.";
            if (C('LOG_RECORD')) Log::write("[$errno] ".$errorStr, Log::ERR);			// link: C() + logCLass
            function_exists('halt') ? halt($errorStr) : exit('ERROR:'.$errorStr);		// link: common/functions.php	halt
            break;
          case E_STRICT:
          case E_USER_WARNING:
          case E_USER_NOTICE:
          default:
            ...
            trace($errorStr, '', 'NOTIC');		// link: common/common.php
            break;
      }
    }
	...
	{
        halt($e->__toString());		// link: common/functions.php	halt
    }
